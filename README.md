...
docker run -it --rm \
    --mount type=bind,src=${PWD},dst=/local --workdir /local \
      ...
      -e CLUSTER_ID="<Zeebe cluster Id>" \
      -e CLUSTER_REGION="<Zeebe region>" \
...
docker run -it --rm \
    --mount type=bind,src=${PWD},dst=/local --workdir /local \
      ...
      -e CLUSTER_HOST="<Zeebe gateway hostname>" \
      -e CLUSTER_PORT="<Zeebe gateway port>" \
      -e CAMUNDA_TENANT_ID="<Optional tenant ID for multi-tenant>" \
      -e PROJECT_TAG="<Optional - Code Repository Tag to Checkout.>" \
      -e CICD_BRANCH="<Optional - Code Branch to Checkout.>" \
      -e GIT_USERNAME="<Git Username>" \
...
| `CLUSTER_HOST`             | The hostname or IP of the Camunda cluster gateway                                                                                                                                                                  | Required for "deploy" operation in Self managed environments                                                                                            |
| `CLUSTER_ID`               | The Id of the Camunda SaaS cluster                                                                                                                                                                                 | Required for "deploy" operation in SaaS environments                                                                                                    |
| `CLUSTER_PORT`             | The port number of the Camunda cluster gateway                                                                                                                                                                     | Optional for "deploy" operation in Self managed environments (default = `26500`)                                                                        |
| `CLUSTER_REGION`           | The region code the cluster is running in                                                                                                                                                                          | Required for "deploy" operation in SaaS environments                                                                                                    |
