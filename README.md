...
| `CAMUNDA_TENANT_ID`        | A comma seperated list of the tenant id(s) to deploy to for multi-tenant environments                          | Optional for "deploy" operations (default no tenants)                                                                                       |
...
| `EXCLUDE`                  | RegEx pattern of path segments to exclude from extraction (Default: wmedIgnore)                                | Optional for "extract" operation                                                                                                            |
...
The Web modeller docker compose does not use these environment variables, so we need to update them individually.
